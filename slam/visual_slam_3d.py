...
class VisualSLAM3D:
    def __init__(
        ...
        print(f"==> Semantic SLAM (YOLO): {use_semantic}")
        ...
        if self.use_semantic:
            self.semantic_filter = SemanticFilter(conf_thresh=0.3)
        else:
    ...
    def process(self):
        ...
        while True:
            ...
            if not pnp_success and p1 is not None and p2 is not None and len(p1) >= 8:
                E, mask_E = cv2.findEssentialMat(p2, p1, self.K, method=cv2.RANSAC, prob=0.999, threshold=0.5)
                # Ensure Homography also maps p2 to p1 (curr to prev) to match recoverPose direction
                H, mask_H = cv2.findHomography(p2, p1, cv2.RANSAC, 3.0)
                ...
                if inliers_H > inliers_E * 0.85 and inliers_H > 15:
                    use_homography = True
                ...
                if use_homography and H is not None:
                    ...
                    if num_solutions > 0:
                        ...
                        print(f"frame {frame_idx}: [Homography] matches={match_count}, inliers={inliers}, ratio={inlier_ratio:.3f}, t={t_vec.round(3)}")
            ...
            if pnp_success:
                ...
                pitch_roll_damp = 0.15 # 0.0=완전고정, 1.0=그대로사용 (평지 주행 시 낮게 설정)
                ...
                self.cur_pose[1, 3] = np.clip(self.cur_pose[1, 3], -0.5, 0.5)
            ...
            if (frame_idx - self.last_keyframe_idx) >= 30:  # 너무 자주 추가되지 않도록 간격 완화
                need_keyframe = True
                    ...
                    if use_flow and len(kpts) > 0:
                        ...
                        for f_idx, mp in self.curr_kpt_to_mp.items():
                            if f_idx < len(flow_p2):
                                ...
                                if dist[best_idx] < 3.0:  # 3픽셀 이내면 동일 특징점으로 간주
                                    aligned_kpt_to_mp[best_idx] = mp
                                    # 이 때 MapPoint의 descriptor 업데이트 (신규 추출된 것으로)
                                    mp.update_descriptor(desc[:, best_idx])
    ...
    def add_keyframe(self, frame_idx, kpts, desc, pts_3d=None, T_rel=None):
        ...
        kf_obj = KeyFrame(frame_idx, self.cur_pose.copy(), kpts, desc)
        for kpt_idx, mp in self.curr_kpt_to_mp.items():
    ...
    def _run_bundle_adjustment(self, kf_ids, num_iterations=10, fix_first=True):
        """
        ...
        # 유효한 키프레임만 필터링
        valid_kfs = [self.map.keyframes[kf_id] for kf_id in kf_ids if kf_id in self.map.keyframes]
        if len(valid_kfs) < 2:
            return
        ...
        # 카메라 파라미터 등록
        cam_params = g2o.CameraParameters(self.focal, [self.cx, self.cy], 0)
        cam_params.set_id(0)
        optimizer.add_parameter(cam_params)
        
        # ID 매핑 (g2o 노드 ID ↔ 실제 객체)
        kf_vertex_ids = {}  # kf_id → g2o_vertex_id
        mp_vertex_ids = {}  # mp_id → g2o_vertex_id
        vertex_to_kf = {}   # g2o_vertex_id → kf_id
        vertex_to_mp = {}   # g2o_vertex_id → mp_id
        
        next_id = 0
        
        # 1. 카메라 포즈 노드 추가 (VertexSE3Expmap)
        for i, kf in enumerate(valid_kfs):
            v = g2o.VertexSE3Expmap()
            v.set_id(next_id)
            
            # 포즈 변환: T_WC (World-to-Camera에서 쓰는 4x4) → SE3Quat
            # KeyFrame.pose는 World coordinate이므로 역변환 필요
            R_wc = kf.pose[:3, :3]
            t_wc = kf.pose[:3, 3]
            ...
            vertex_to_kf[next_id] = kf.id
            next_id += 1
        
        # 2. MapPoint 노드 추가 (VertexPointXYZ)
        for mp_id, mp in involved_mps.items():
            v = g2o.VertexPointXYZ()
            v.set_id(next_id)
            v.set_estimate(mp.pos3d)
            v.set_marginalized(True)  # Schur complement 적용
            
            optimizer.add_vertex(v)
            mp_vertex_ids[mp_id] = next_id
            vertex_to_mp[next_id] = mp_id
            next_id += 1
        
        # 3. 재투영 엣지 추가 (EdgeProjectXYZ2UV)
        kf_id_set = set(kf_ids)
        edge_count = 0
        ...
        for mp_id, mp in involved_mps.items():
            ...
            for obs_kf_id, pixel_uv in mp.observations.items():
                if obs_kf_id not in kf_id_set:
                    continue
                if obs_kf_id not in kf_vertex_ids:
                    continue
                ...
                kernel.set_delta(5.991)  # chi2 95% threshold for 2DOF
                e.set_robust_kernel(kernel)
                e.set_parameter_id(0, 0)  # 카메라 파라미터 바인딩
    ...
    def run_local_ba(self):
        ...
        num_recent = 5
        recent_kf_ids = sorted(self.map.keyframes.keys(), reverse=True)[:num_recent]
        if len(recent_kf_ids) < 2:
            return
        self._run_bundle_adjustment(recent_kf_ids, num_iterations=5, fix_first=True)

    def run_global_ba(self):
        """Global Bundle Adjustment: 전체 키프레임 + 전체 MapPoint 최적화"""
        ...
        for i in range(len(self.keyframes)):
            self.keyframes[i][1, 3] = np.clip(self.keyframes[i][1, 3], -0.5, 0.5)
        # Map 내 KeyFrame 포즈도 동기화
        for kf_id, kf in self.map.keyframes.items():
            kf.pose[1, 3] = np.clip(kf.pose[1, 3], -0.5, 0.5)
        
        print("===> Global BA Complete.")
    ...
    def visualize_final_result(self):
        ...
        if not rebuilt_points:
            ...
            print(f" -> Rendering {len(points)} points from Map across {len(self.keyframes)} keyframes.")
